...
public class LendBiz {
	...
	public void ret(Lend lend) throws BizException {
		// 开始事务管理
		DBHelper dbh = new DBHelper(true);
		Connection conn = null;
		try {
			...
			conn = dbh.getConnection();
			// 更新借阅记录的还书时间字段
			String sql = "update lend set rettime = sysdate where id = ?";
			dbh.update(sql, lend.getId());
			sql = "update book set status=0 where id=?";
			dbh.update(sql, lend.getBookid());
			// 提交事务
			conn.commit();
		} catch (Exception e) {
